...
#  point = np.array(point)
  pos = []
  for i in range(133):
      dot_products = np.dot(np.stack(db[i]['Embeddings']), query_embedding["embedding"])
      idx = np.argmax(dot_products)  # Lấy chỉ số cao nhất
      pos.append(idx)
      point.append(dot_products[idx])
  id = np.argsort(point)[-n:][::-1]  # Lấy n chỉ số cao nhất
  for k in range(len(id)):
     result.append((id[k],pos[id[k]]))
  return result
def relevant_passage1(query):
  relevant_passage1 = ""
  pair = find_best_passage(query, db)
  for i in range(len(pair)):
    passage = db[pair[i][0]].iloc[pair[i][1]]['Text']
    relevant_passage1 += "Điều " + str(pair[i][0]+1) + " " + "Khoản " + passage + "\n"
  return relevant_passage1
def relevant_context(text):
   relevant = []
   # Biểu thức chính quy để tìm nội dung được bao bọc bởi ** ở đầu và :** ở cuối
   pattern = r'\*\*(.*?)\*\*'
   # Tìm tất cả các khớp
   matches = re.findall(pattern, text)
   for i in matches:
     # Biểu thức chính quy để lấy nội dung sau ". "
     match = re.search(r'Điều\s(\S+)', i)
  # Nếu tìm thấy khớp, in ra kết quả
     if match:
       result = match.group(1)
       check = result[-1]
       if(check >= '0' and check <= '9'):
         relevant.append(int(result))
       else:
         relevant.append(int(result[:-1]))
   return relevant
def relevant_context0(text):
   relevant = []
   matches = text.split(" ")
   for result in matches:
     if result:
       ...
       if(check >= '0' and check <= '9'):
          check2 = result[-1]
          if(check2 >= '0' and check2 <= '9'):
              relevant.append(int(result))
          else:
              relevant.append(int(result[:-1]))
   return relevant
def relevant_passage(input):
   relevant = ""
   for i in input:
     passage = columns[i-1]
     relevant += "\nĐiều" + str(i) + " " + passage
     if(len(law_dict[passage]) == 1):
       relevant +=" "+str(law_dict[passage][0])
     else:
...
def make_first_prompt_gt(query, relevant_passage):
  ...
  Đảm bảo trả lời bằng một câu hoàn chỉnh, toàn diện, bao gồm tất cả thông tin cơ bản có liên quan.\
  Biết đoạn văn bạn tham khảo thuộc Nghị định 168/2024/NĐ-CP, và nêu rõ bạn tham khảo điều này, khoản nào, điểm nào trong đoạn văn đó\
  Tuy nhiên, bạn đang nói chuyện với người dùng không biết nhiều về pháp luật, vì vậy hãy nhớ chia nhỏ các khái niệm phức tạp và\
  tạo ra một giọng điệu thân thiện và mang tính đối thoại.\
  Nếu đoạn văn không liên quan đến câu trả lời,hãy trả lời bằng những kiến thức bạn có.
  QUESTION: '{query}'
